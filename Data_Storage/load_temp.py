# this program loads Census ACS data using a temporary table and memory tuning
...
def getSQLcmnds(rowlist):
	...
	for row in rowlist:
		valstr = row2vals(row)
		cmd = f"INSERT INTO stagging VALUES ({valstr});"
		cmdlist.append(cmd)
	return cmdlist
...
def createTable(conn):
	...
	with conn.cursor() as cursor:
		...
		print(f"Created stagging table")
...
def load(conn, icmdlist):
    with conn.cursor() as cursor:
        ...
        cursor.execute(f"CREATE TABLE {TableName} AS TABLE stagging;")
