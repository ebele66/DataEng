	...
	with conn.cursor() as cursor:
		cursor.execute(f"""
            ...
            DROP TABLE IF EXISTS stagging
         	CREATE UNLOGGED TABLE stagging (
            	Year                INTEGER,
                CensusTract         NUMERIC,
            	State               TEXT,
            	County              TEXT,
		...
		print(f"Created {TableName} and stagging")
...
def load(conn, icmdlist):
    with conn.cursor() as cursor:
        ...
        cursor.execute(f"Insert into {TableName} (select * from stagging);")
        cursor.execute(f"commit;")
        cursor.execute(f"drop table stagging;")
